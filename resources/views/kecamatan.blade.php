  ...
  <section class="container mx-auto p-6">
    ...
    <nav class="text-sm mb-6">
      <ol class="flex flex-wrap text-gray-600">
        ...
        <li class="mx-2">></li>
        <li><a href="#" class="hover:text-blue-600">Kecamatan {{ $kecamatan->nama_kecamatan }}</a></li>
      </ol>
    </nav>

    <!-- Daftar Desa -->
    <div class="bg-white shadow-lg rounded-2xl p-6">
        <h2 class="text-xl font-semibold mb-4">Daftar Desa & Kelurahan di Kecamatan {{ $kecamatan->nama_kecamatan }}</h2>
        <ul class="space-y-4">
        <!-- Desa -->
        @foreach ($kecamatan->desa as $desa)
        <li class="flex items-center justify-between border-b pb-3">
            <div>
                <p class="font-semibold">{{ $desa->nama_desa }}</p>
                <!-- <p class="text-sm text-gray-600">{{ $desa->nama_desa }}</p> -->
            </div>
            <a href="/desa/{{ $desa->id }}" 
